...
export function createPreview(inventory, data, targetDir = dynamicDir) {
  ...
  const nativeSource = data.datasets.some(x => x.kind === 'raw-postgres-native-query');
  const desktopSource = data.datasets.some(x => x.kind === 'desktop-model-export');
...
<div class="status" id="report-status">${noData ? 'PBIP metadata only: no readable data source found. Values and charts cannot be verified.' : desktopSource ? 'Loaded Power BI Desktop model tables, including Power Query results. DAX measures and visual parity still require validation. Static HTML embeds every exported table: check for confidential fields before sharing.' : nativeSource ? 'PostgreSQL native SQL results loaded. Later Power Query steps, merges, and DAX are not automatically reproduced; values may differ from Power BI.' : rawSource || postgresSource ? 'Direct CSV/PostgreSQL source read. Rows are raw; Power Query transformations and DAX results are not automatically reproduced.' : 'Local data supplied. Visual calculations and fidelity still require review.'}</div>
