...
The converter scans the PBIP, starts three fresh Gemini CLI sessions (interpret, build, independent review), and generates `output/dynamic/index.html` plus a source-specific local `output/dynamic/backend.mjs`. It then runs the generated source healthcheck and calls every visual endpoint. Only after those checks pass does it serve the generated report at `http://127.0.0.1:8765/`. These checks do not prove that the generated source logic is correct. `npm run preflight` checks PBIP structure without Gemini or source access. The old PostgreSQL table browser is available separately as `npm run live-preview`; it is not the report converter. The older static export path remains `npm run convert` but is not the default.
