...
// The self-test's own report lives here (short paths); development fixtures live in tests/fixtures.
export const demoDir = path.join(geminiDir, 'selftest', 'Demo');
export const fixturesDir = path.join(geminiDir, 'tests', 'fixtures');

export function createSandbox({ fixture = 'Demo', env = {} } = {}) {
  ...
  const source = fixture === 'Demo' ? demoDir : path.join(fixturesDir, fixture);
